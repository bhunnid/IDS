"""
app.py  —  Ulinzi HIDS  ·  Web Dashboard + Engine Launcher  (v3)
=================================================================
Run:  sudo python3 app.py
Open: http://localhost:5000

sudo is required — the engine opens a raw AF_PACKET socket and reads
/var/log/auth.log. The engine starts automatically on launch.
"""
...
# ── Auto-start engine ─────────────────────────────────────────────────────────
threading.Thread(target=engine_mod.start_engine, daemon=True).start()
...
_LEVEL_BG = {
    "CRITICAL": "rgba(255,61,87,.09)",
    "HIGH":     "rgba(255,107,53,.09)",
    "MEDIUM":   "rgba(245,158,11,.09)",
    "LOW":      "rgba(56,178,255,.09)",
...
def _parse(raw: str):
    ...
    display = rule[5:] if rule.startswith("scan_") else rule
    meta    = _RULE_META.get(display, ("Alert", "⚠"))
    label   = f"Port Scan ({rule[5:]})" if rule.startswith("scan_") else meta[0]
    icon    = "🔍" if rule.startswith("scan_") else meta[1]
    return {
        "ts":     ts,
        "level":  level,
        "rule":   rule,
...
def _counts():
    ...
    if not os.path.exists(ALERT_LOG): return c
    try:
        with open(ALERT_LOG, errors="replace") as fh:
            for line in fh:
                a = _parse(line)
                if a and a["level"] in c: c[a["level"]] += 1
    except OSError: pass
    return c
...
def _category_counts():
    ...
    h = n = 0
    if not os.path.exists(ALERT_LOG): return {"host":0,"network":0}
    try:
        with open(ALERT_LOG, errors="replace") as fh:
            for line in fh:
                a = _parse(line)
                if not a or a["level"] == "INFO": continue
                r = a["rule"]
...
def _spark(buckets: int = 30, secs_per_bucket: int = 60):
    result = [0] * buckets
    if not os.path.exists(ALERT_LOG): return result
    now = datetime.now()
    try:
        with open(ALERT_LOG, errors="replace") as fh:
            for line in fh:
...
.hdr{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:.9rem;border-bottom:1px solid var(--brd)}
.brand{display:flex;align-items:center;gap:.7rem}
.ring{width:11px;height:11px;border-radius:50%;border:2px solid var(--blu);box-shadow:0 0 10px var(--blu);animation:pulse 2s ease-in-out infinite}
...
.pill-info::before{background:var(--mut)}.pill-host::before{background:var(--org)}
.pill-net::before{background:var(--cyn)}.pill-win::before{background:var(--vio)}
...
.card-hdr{padding:.55rem .9rem;border-bottom:1px solid var(--brd);font-family:var(--font);font-size:.58rem;letter-spacing:.14em;text-transform:uppercase;color:var(--mut);display:flex;justify-content:space-between;align-items:center}
.card-body{padding:.9rem}
/* badge */
.badge{display:flex;align-items:center;gap:.45rem;padding:.38rem .7rem;border-radius:4px;font-family:var(--font);font-size:.72rem;letter-spacing:.06em;text-transform:uppercase;border:1px solid;margin-bottom:.8rem;justify-content:center}
...
.dot-stopped{background:var(--red)}.dot-baseline{background:var(--amb);animation:blink .7s step-end infinite}
.dot-detecting{background:var(--grn);box-shadow:0 0 8px var(--grn)}
@keyframes blink{50%{opacity:0}}
/* progress */
.prog-wrap{margin-bottom:.8rem}.prog-hidden{display:none!important}
.prog-labels{display:flex;justify-content:space-between;font-family:var(--font);font-size:.62rem;color:var(--mut);margin-bottom:.22rem}
.prog-track{height:5px;background:var(--dim);border-radius:3px;overflow:hidden}
.prog-bar{height:100%;background:linear-gradient(90deg,var(--amb),var(--blu));border-radius:3px;transition:width .6s ease}
/* monitor grid */
.mon-grid{display:grid;grid-template-columns:1fr 1fr;gap:.35rem;margin-bottom:.8rem}
.mon-pill{background:var(--bg2);border:1px solid var(--brd);border-radius:4px;padding:.28rem .5rem;font-family:var(--font);font-size:.6rem}
...
.tbar{background:var(--dim);border-radius:3px;overflow:hidden;height:4px}
...
.btn{width:100%;padding:.5rem;border:1px solid;border-radius:4px;font-family:var(--ui);font-size:.8rem;font-weight:600;letter-spacing:.04em;text-transform:uppercase;cursor:pointer;transition:opacity .12s,box-shadow .12s;background:none}
...
.flash{margin-top:.6rem;padding:.35rem .55rem;border-radius:4px;font-family:var(--font);font-size:.68rem;border:1px solid}
...
.live-dot{width:6px;height:6px;border-radius:50%;background:var(--grn);box-shadow:0 0 6px var(--grn);animation:blink .9s step-end infinite}
...
.fbtn{background:none;border:1px solid var(--brd);border-radius:3px;color:var(--mut);font-family:var(--font);font-size:.63rem;padding:.12rem .44rem;cursor:pointer}
.fbtn:hover{color:var(--txt);border-color:var(--brd2)}
.filters{display:flex;gap:.28rem;flex-wrap:wrap}
.ftab{background:none;border:1px solid var(--brd);border-radius:3px;color:var(--mut);font-family:var(--font);font-size:.6rem;padding:.12rem .4rem;cursor:pointer;text-transform:uppercase;letter-spacing:.05em}
.ftab.on{color:var(--txt);border-color:var(--brd2);background:var(--brd)}
.feed{max-height:calc(100vh - 260px);min-height:320px;overflow-y:auto;padding:.35rem;scrollbar-width:thin;scrollbar-color:var(--brd) transparent}
.feed::-webkit-scrollbar{width:3px}
.feed::-webkit-scrollbar-thumb{background:var(--brd);border-radius:2px}
.acard{display:grid;grid-template-columns:32px 1fr auto;gap:.45rem;align-items:start;padding:.55rem .65rem;border-radius:5px;border:1px solid var(--brd);border-left:3px solid;margin-bottom:.35rem;animation:fadein .18s ease;font-family:var(--font);font-size:.68rem}
...
.ac-level{font-size:.58rem;font-weight:700;letter-spacing:.1em;padding:.05rem .3rem;border-radius:2px;text-transform:uppercase}
...
.empty{display:flex;flex-direction:column;align-items:center;justify-content:center;height:220px;color:var(--dim);font-family:var(--font);font-size:.72rem;gap:.4rem}
...
<header class="hdr">
  <div class="brand">
    ...
    <div>
      ...
      <div class="brand-sub">HOST INTRUSION DETECTION SYSTEM · v3 · KALI LINUX</div>
    </div>
  </div>
  <div class="hdr-r">
    <div id="clock"></div>
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div id="prog-wrap" class="prog-wrap {{ '' if phase == 'baseline' else 'prog-hidden' }}">
        <div class="prog-labels"><span>Baseline learning</span><span id="prog-pct">{{ "%.0f"|format(bpct*100) }}%</span></div>
        <div class="prog-track"><div id="prog-bar" class="prog-bar" style="width:{{ "%.1f"|format(bpct*100) }}%"></div></div>
      </div>
      <div class="traffic-bar-wrap">
        <div class="traffic-lbl">Live inbound traffic (pkts/s)</div>
        <div class="traffic-bars">
          <div><div class="tbar"><div id="tb-syn"  class="tbar-fill" style="width:0%;background:var(--red)"></div></div><div class="tbar-label"><span>SYN</span><span id="tv-syn">0</span></div></div>
          <div><div class="tbar"><div id="tb-udp"  class="tbar-fill" style="width:0%;background:var(--org)"></div></div><div class="tbar-label"><span>UDP</span><span id="tv-udp">0</span></div></div>
          <div><div class="tbar"><div id="tb-icmp" class="tbar-fill" style="width:0%;background:var(--amb)"></div></div><div class="tbar-label"><span>ICMP</span><span id="tv-icmp">0</span></div></div>
          <div><div class="tbar"><div id="tb-tot"  class="tbar-fill" style="width:0%;background:var(--blu)"></div></div><div class="tbar-label"><span>TOT</span><span id="tv-tot">0</span></div></div>
        </div>
      </div>
      <div class="mon-grid">
        <div class="mon-pill"><div class="mon-lbl">Auth Log</div><div class="mon-val" id="mon-auth"><div class="mdot {{ 'mon-on' if mon.auth_log else 'mon-off' }}"></div>{{ 'Active' if mon.auth_log else 'N/A' }}</div></div>
        <div class="mon-pill"><div class="mon-lbl">Process Mon.</div><div class="mon-val"><div class="mdot {{ 'mon-on' if mon.psutil else 'mon-off' }}"></div>{{ 'Active' if mon.psutil else 'No psutil' }}</div></div>
        <div class="mon-pill"><div class="mon-lbl">File Integrity</div><div class="mon-val"><div class="mdot mon-on"></div>{{ mon.fim_files }} files</div></div>
        <div class="mon-pill"><div class="mon-lbl">Network Cap.</div><div class="mon-val"><div class="mdot {{ 'mon-on' if running else 'mon-off' }}"></div><span id="mon-iface">{{ mon.iface }}</span></div></div>
      </div>
      <table class="meta">
      ...
      <div class="btns">
        <form method="POST" action="/start" style="margin:0"><button class="btn btn-start" {{ 'disabled' if running else '' }}>▶ Start</button></form>
        <form method="POST" action="/stop"  style="margin:0"><button class="btn btn-stop"  {{ 'disabled' if not running else '' }}>■ Stop</button></form>
      </div>
      <form method="POST" action="/clear" style="margin:0"><button class="btn btn-clear">⊘ Clear alert log</button></form>
      {% if flash %}<div class="flash {{ 'flash-ok' if ok else 'flash-err' }}">{{ flash }}</div>{% endif %}
    </div>
  </div>
...
function drawSpark(d){
  ...
  d.forEach((v,i)=>{
    ...
    ctx.fillStyle=t>.7?`rgba(255,61,87,${.45+t*.45})`:t>.35?`rgba(245,158,11,${.3+t*.5})`:`rgba(56,178,255,${.18+t*.5})`;
...
let activeF='ALL';
function setF(f,el){activeF=f;document.querySelectorAll('.ftab').forEach(b=>b.classList.remove('on'));el.classList.add('on');applyF();}
function applyF(){document.querySelectorAll('#feed .acard').forEach(c=>{c.style.display=(activeF==='ALL'||c.dataset.lv===activeF)?'':'none';});}
let paused=false;
function togglePause(){paused=!paused;document.getElementById('pbtn').textContent=paused?'Resume':'Pause';document.getElementById('live-dot').classList.toggle('paused',paused);}
function esc(s){return String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
const PHASES={stopped:'Stopped',baseline:'Baseline…',detecting:'Detecting'};
const LVLBG={CRITICAL:'rgba(255,61,87,.09)',HIGH:'rgba(255,107,53,.09)',MEDIUM:'rgba(245,158,11,.09)',LOW:'rgba(56,178,255,.09)',INFO:'rgba(74,106,133,.06)'};
const LVLCOL={CRITICAL:'#ff3d57',HIGH:'#ff6b35',MEDIUM:'#f59e0b',LOW:'#38b2ff',INFO:'#4a6a85'};
function buildCard(a){
  const bg=LVLBG[a.level]||'rgba(100,100,100,.06)',col=LVLCOL[a.level]||'#888';
  return `<div class="acard" data-lv="${esc(a.level)}" style="border-left-color:${col};background:${bg}">
    <div class="ac-icon">${esc(a.icon||'⚠')}</div>
    <div><div class="ac-top"><span class="ac-level level-${esc(a.level)}">${esc(a.level)}</span><span class="ac-label" style="color:${col}">${esc(a.label)}</span></div>
    <div class="ac-detail">${esc(a.detail)}</div></div>
    <div class="ac-ts">${esc(a.ts?a.ts.slice(11):'')}</div></div>`;
}
...
async function poll(){
  try{
    const[sr,ar,mr]=await Promise.all([fetch('/status'),fetch('/alerts?n=120'),fetch('/metrics')]);
    ...
    const hp=document.getElementById('hdr-phase');if(hp)hp.textContent=PHASES[st.phase]||st.phase;
    const uEl=document.getElementById('mt-uptime');if(uEl)uEl.textContent=st.uptime||'—';
    const pw=document.getElementById('prog-wrap'),pb=document.getElementById('prog-bar'),pp=document.getElementById('prog-pct');
    if(st.phase==='baseline'){
      pw.classList.remove('prog-hidden');
      const p=Math.min((st.baseline_pct||0)*100,99);
      if(pb)pb.style.width=p.toFixed(1)+'%';if(pp)pp.textContent=p.toFixed(0)+'%';
    }else{pw.classList.add('prog-hidden');}
    const wEl=document.getElementById('c-win');if(wEl)wEl.textContent=st.windows||0;
    const c=mt.counts||{};
    ...
    if(hEl)hEl.textContent=cat.host??0;if(nEl)nEl.textContent=cat.network??0;
    ...
    if(al.alerts&&al.alerts.length){
      ...
      const lEl=document.getElementById('mt-last');if(lEl)lEl.textContent=first?first.ts.slice(11):'—';
    }
    if(!paused&&al.alerts){
      const feed=document.getElementById('feed'),cntEl=document.getElementById('feed-count');
...
def _render(flash: str = "", ok: bool = True):
    ...
    return render_template_string(
        ...
        flash         = flash,
        ok            = ok,
        mon           = {"auth_log":mon.get("auth_log",False),"psutil":mon.get("psutil",False),
                         "fim_files":mon.get("fim_files",0),"iface":mon.get("iface","—")},
...
@app.get("/")
def index(): return _render()

@app.post("/start")
def start():
    if engine_mod.is_running(): return _render("Engine already running.", ok=False)
    engine_mod.start_engine()
    return _render("Engine started.", ok=True)

@app.post("/stop")
def stop():
    if not engine_mod.is_running(): return _render("Engine not running.", ok=False)
    engine_mod.stop_engine()
...
def clear():
    try:
        if os.path.exists(ALERT_LOG): os.replace(ALERT_LOG, ALERT_LOG + ".bak")
        return _render(f"Log cleared (backup → {ALERT_LOG}.bak).", ok=True)
    except OSError as e:
...
@app.get("/metrics")
def metrics():
    return jsonify({"counts":_counts(),"cat":_category_counts(),"spark":_spark()})
...
if __name__ == "__main__":
    ...
    print(f"  Engine starting automatically...\n")
